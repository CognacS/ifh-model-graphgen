...
#   which is adapted from https://github.com/lrjconan/GRAN/
#   which is adapted from https://github.com/JiaxuanYou/graph-generation
#
###############################################################################
...
try:
    ...
    _GRAPH_TOOL_IMPORTED = True
except ImportError as e:
    #print(e)
    #print('graph-tool not installed, some functions will not be available')
    _GRAPH_TOOL_IMPORTED = False
...
from src.metrics.utils.distributions import compute_mmd, gaussian_emd, gaussian, emd, gaussian_tv, disc
...
def is_sbm_graph(G, p_intra=0.3, p_inter=0.005, strict=True, refinement_steps=1000):
    ...
    if not _GRAPH_TOOL_IMPORTED:
        print('Graph tool not imported, cannot check SBM, returning graph not valid')
        return False
...
def graph_non_isomorphic_to_list(g, g_list):
    for g2 in g_list:
        if nx.faster_could_be_isomorphic(g, g2):
...
def eval_vun(fake_graphs, train_graphs, validity_func=(lambda x: True)):
    num_graphs = len(fake_graphs)
    if num_graphs == 0:
        return 0, 0, 0
    ...
    for fake_g in fake_graphs:
        ...
        unique = graph_non_isomorphic_to_list(fake_g, fake_evaluated)

        if unique:
            count_unique += 1
            fake_evaluated.append(fake_g)

            novel = graph_non_isomorphic_to_list(fake_g, train_graphs)

            if novel:
                count_novel += 1

                valid = validity_func(fake_g)

                if valid:
                    count_valid += 1

    frac_unique = count_unique / num_graphs         # Fraction of distinct isomorphism classes in the fake graphs
    frac_unique_novel = count_novel / num_graphs    # Fraction of distinct isomorphism classes in the fake graphs that are not in the training set
    frac_vun = count_valid / num_graphs             # Fraction of distinct isomorphism classes in the fake graphs that are not in the training set and are valid
    return frac_unique, frac_unique_novel, frac_vun


def dataloader_to_nx(dataloader) -> List[nx.Graph]:
    networkx_graphs = []
